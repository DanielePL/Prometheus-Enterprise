-- Coach Invitations (clean slate)

DROP TABLE IF EXISTS coach_invitations CASCADE;
...
CREATE TRIGGER update_coach_invitations_updated_at BEFORE UPDATE ON coach_invitations FOR EACH ROW EXECUTE FUNCTION update_updated_at();

ALTER TABLE coach_invitations ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view invitations for their gym" ON coach_invitations FOR SELECT
USING (gym_id IN (SELECT gym_id FROM profiles WHERE id = auth.uid()));
CREATE POLICY "Users can create invitations for their gym" ON coach_invitations FOR INSERT
WITH CHECK (gym_id IN (SELECT gym_id FROM profiles WHERE id = auth.uid()));
CREATE POLICY "Users can update invitations for their gym" ON coach_invitations FOR UPDATE
USING (gym_id IN (SELECT gym_id FROM profiles WHERE id = auth.uid()));
